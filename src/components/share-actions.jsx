    ...
    >
      {hasPdf ? (
        <button
          ...
          onClick={onPDF}
          className={cn(
            "inline-flex items-center justify-center gap-1.5 rounded-full font-semibold transition-[opacity,transform] duration-200 ease-out hover:opacity-90 active:scale-[0.98]",
            tight ? "h-11 px-2 text-xs" : "h-12 px-4 text-sm",
            "bg-[var(--forest)] text-white dark:bg-[var(--lime)] dark:text-[var(--forest)]"
          )}
        >
          <FileText className={tight ? "size-3.5" : "size-4"} />
          {t("common.pdf")}
        </button>
      ) : null}
      <button
