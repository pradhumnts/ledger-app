...
export function SoftCard({ children, className = "" }) {
  return (
    <div
      className={cn(
        "overflow-hidden rounded-[1.75rem] border border-black/[0.04] bg-white shadow-[0_10px_40px_rgba(0,0,0,0.04)] transition-shadow duration-300 dark:border-white/10 dark:bg-zinc-900 dark:shadow-none",
        className
      )}
    >
      {children}
...
export function ListRow({
  ...
  const classes = cn(
    "flex w-full items-center gap-3 px-4 py-3.5 text-left transition-[background-color,transform] duration-200 ease-out hover:bg-zinc-50/80 active:scale-[0.995] dark:hover:bg-zinc-800/60",
    className
  );
